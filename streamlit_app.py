...
def load_and_train_model():
    ...
    # NYC is too large for a single Overpass query (~34 MB response gets truncated).
    # Fetch each borough separately and combine, then cache to disk so the
    # download only happens once across container restarts.
    GRAPH_CACHE = 'data/nyc_graph.graphml'
    try:
        if os.path.exists(GRAPH_CACHE):
            nyc_graph = ox.load_graphml(GRAPH_CACHE)
        else:
            boroughs = [
            ...
            graphs = [ox.graph_from_place(b, network_type='drive', simplify=True) for b in boroughs]
            nyc_graph = functools.reduce(nx.compose, graphs)
            os.makedirs('data', exist_ok=True)
            ox.save_graphml(nyc_graph, GRAPH_CACHE)
...
if model is not None:
    ...
    # Using a sample of segments to get a city-wide representative prediction
    ...
    sns.barplot(data=trend_df, x='hour', y='avg_vol', hue='hour', palette='coolwarm', legend=False, ax=ax2)
